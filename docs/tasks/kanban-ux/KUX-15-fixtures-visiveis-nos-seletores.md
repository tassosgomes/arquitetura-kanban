...
- [x] Fazer os testes de integração limparem o que criam, ou rodarem contra um banco
      descartável isolado do banco de desenvolvimento.
- [x] Nomear fixtures de forma determinística e sem colisão, de modo que uma execução
      repetida não acumule duplicatas (`T12 actor` ×3 é acúmulo, não coincidência).
- [x] Limpar os registros já existentes na base de desenvolvimento/homologação.
- [x] Documentar no guia de desenvolvimento local qual banco os testes de integração
...
- [x] Nenhuma opção de "Responsável", "Participantes" ou "Categoria" contém `T06`,
      `T12`, `fixture` ou um UUID no rótulo.
- [x] Rodar a suíte de integração duas vezes seguidas não aumenta a contagem de
      usuários nem de categorias visíveis na aplicação.
- [x] A suíte continua passando após o isolamento.
- [x] O guia de desenvolvimento local descreve o banco usado pelos testes.
